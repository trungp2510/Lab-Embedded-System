#include "TaskWebServer.h"
#include <WiFi.h>
#include <WebServer.h>
#include <Preferences.h>
unsigned long connect_start_ms = 0;
bool connecting = false;
bool isAPMode = true;
// Biến toàn cục nội bộ
static Preferences prefs;
static WebServer* server = nullptr;
static String current_ap_ssid;
static String current_ap_pass;

// HTML giao diện
String mainPage()
{

  return R"rawliteral(
  <!DOCTYPE html>
  <html lang="vi">
  <head>
    <meta charset="UTF-8">
    <meta name="viewport" content="width=device-width, initial-scale=1.0">
    <title>ESP32 Dashboard</title>
    <style>
      body {
        font-family: "Segoe UI", Arial, sans-serif;
        background: #f2f3f5;
        color: #333;
        text-align: center;
        margin: 0;
        height: 100vh;
        display: flex;
        flex-direction: column;
        align-items: center;
        justify-content: center;
      }

      .logo {
        width: 220px;
        height: 90px;
        border-radius: 20px;
        box-shadow: 0 4px 10px rgba(0,0,0,0.2);
        margin-bottom: 25px;
        background: #f2f3f5;
        object-fit: contain;
        padding: 10px 15px;
      }

      .container {
        background: linear-gradient(135deg, #1e90ff, #00bfff);
        padding: 35px 45px;
        border-radius: 20px;
        box-shadow: 0 4px 25px rgba(0,0,0,0.15);
        width: 90%;
        max-width: 400px;
        color: #fff;
        backdrop-filter: blur(6px);
      }

      h1 {
        font-size: 1.8em;
        margin-bottom: 20px;
      }

      .sensor {
        font-size: 1.1em;
        margin: 10px 0;
      }

      .sensor span {
        font-weight: bold;
        color: #00ffcc;
      }

      button {
        margin: 10px;
        background: #00ffcc;
        color: #000;
        font-weight: bold;
        border: none;
        border-radius: 20px;
        padding: 10px 20px;
        cursor: pointer;
        transition: all 0.3s;
        font-size: 1em;
      }

      button:hover {
        background: #00e0b0;
        transform: scale(1.05);
      }

      #settings {
        background: #f2f3f5;
        color: #007bff;
        font-weight: bold;
      }

      #settings:hover {
        background: #e6f3ff;
      }
    </style>
  </head>

  <body>
    <img src="data:image/png;base64," alt="Project Icon" class="logo">
    <div class="container">
      <h1>📊 ESP32 Dashboard</h1>
      <div class="sensor">
        🌡️ Nhiệt độ: <span id="temp">)rawliteral" +
         String(global_temperature) + R"rawliteral(</span> &deg;C
      </div>
      <div class="sensor">
        💧 Độ ẩm: <span id="hum">)rawliteral" +
         String(global_humidity) + R"rawliteral(</span> %
      </div>


      <button id="settings" onclick="window.location='/settings'">⚙️ Cài đặt</button>
    </div>

    <script>
      function toggleLED(id) {
        fetch('/toggle?led='+id)
          .then(response=>response.json())
          .then(json=>{
            document.getElementById('l1').innerText=json.led1;
            document.getElementById('l2').innerText=json.led2;
          });
      }

      setInterval(()=>{
        fetch('/sensors')
          .then(res=>res.json())
          .then(d=>{
            document.getElementById('temp').innerText=d.temp;
            document.getElementById('hum').innerText=d.hum;
          });
      },3000);
    </script>
  </body>
  </html>
  )rawliteral";
}

String settingsPage()
{
  return R"rawliteral(
  <!doctype html>
<html lang="vi">
<head>
<meta charset="utf-8">
<meta name="viewport" content="width=device-width, initial-scale=1, viewport-fit=cover">
<title>Kết nối Wi-Fi</title>
<style>
  :root{
    --bg:#ffffff; --panel:#ffffff; --card:#ffffff; --muted:#6b7280; --accent:#2563eb; --text:#0f172a; --line:#e5e7eb;
    --wifi:#0f172a; --wifi-muted:#cbd5e1; --lock:#0f172a;
  }
  *{box-sizing:border-box}
  html,body{height:100%}
  body{
    margin:0; background:var(--bg); color:var(--text);
    font-family: system-ui, -apple-system, Segoe UI, Roboto, Arial, sans-serif;
    -webkit-font-smoothing:antialiased;
  }
  .app{max-width:720px; margin:0 auto; padding-bottom: env(safe-area-inset-bottom, 16px);}
  .header{
    position:sticky; top:0; z-index:10; background:var(--panel);
    padding:14px 16px; border-bottom:1px solid var(--line);
    display:flex; align-items:center; gap:10px;
  }
  .title{font-size:1.05rem; font-weight:700}
  .ip{font-size:.85rem; color:var(--muted); padding:2px 8px; border:1px solid var(--line); border-radius:999px;}
  .btn{
    appearance:none; border:1px solid var(--line); background:#f8fafc; color:var(--text);
    padding:10px 14px; border-radius:12px; font-weight:600; cursor:pointer; margin-left:auto;
  }
  .content{padding:14px 16px}
  .section-label{font-size:.85rem; color:var(--muted); margin:8px 4px}
  .list{display:flex; flex-direction:column; gap:8px}
  .cell{
    background:var(--card); border:1px solid var(--line); border-radius:14px;
    padding:12px 14px; display:flex; align-items:center; gap:12px;
    touch-action:manipulation;
  }
  .cell:active{transform:scale(.995)}
  .wifi-icon{position:relative; width:28px; height:22px; flex:0 0 28px;}
  .lock{
    position:absolute; right:-4px; bottom:-2px; width:14px; height:14px;
    color:var(--lock); opacity:.85;
  }
  .ssid{font-weight:700}
  .meta{font-size:.85rem; color:var(--muted)}
  .grow{flex:1}
  .rssi{font-size:.85rem; color:var(--muted); min-width:64px; text-align:right}
  .empty{
    text-align:center; color:var(--muted); padding:24px; border:1px dashed var(--line);
    border-radius:16px; background:#fafafa; margin-top:8px;
  }
  .sheet-backdrop{position:fixed; inset:0; background:rgba(0,0,0,.25); display:none; align-items:flex-end; z-index:50;}
  .sheet{width:100%; max-width:720px; margin:0 auto; background:var(--panel);
    border-top-left-radius:18px; border-top-right-radius:18px; border:1px solid var(--line);
    transform:translateY(100%); transition:transform .25s ease;
  }
  .sheet.open{transform:translateY(0)}
  .sheet-head{padding:14px 16px; border-bottom:1px solid var(--line); display:flex; justify-content:space-between; align-items:center}
  .x{border:none; background:transparent; font-size:1.25rem; color:var(--muted); cursor:pointer}
  .sheet-body{padding:16px}
  label{display:block; font-size:.9rem; margin:.6rem 0 .35rem}
  input{width:100%; padding:12px; border-radius:12px; border:1px solid var(--line); background:#fff; color:var(--text); font-size:1rem;}
  .help{font-size:.85rem; color:var(--muted); margin-top:6px}
  .footer{padding:14px 16px; border-top:1px solid var(--line); display:flex; justify-content:flex-end; gap:8px}
  .btn2{appearance:none; border:1px solid var(--line); background:#fff; color:var(--text); padding:10px 14px; border-radius:12px; font-weight:600; cursor:pointer;}
  .btn2.primary{background:var(--accent); border:none; color:#fff;}
  .spinner{width:16px;height:16px;border:2px solid var(--line);border-top-color:var(--accent);border-radius:50%;animation:spin .8s linear infinite;display:inline-block;vertical-align:middle}
  @keyframes spin{to{transform:rotate(360deg)}}
</style>
</head>
<body>
<div class="app">
  <div class="header">
    <div class="title">Kết nối Wi-Fi</div>
    <div class="ip" id="ip">IP: %IP%</div>
    <button class="btn" id="btnRefresh" title="Quét lại">Quét</button>
  </div>

  <div class="content">
    <div class="section-label">Mạng khả dụng</div>
    <div id="list" class="list"></div>
    <div id="empty" class="empty" style="display:none">Không thấy mạng nào. Bấm "Quét".</div>
  </div>
</div>

<div id="sheetWrap" class="sheet-backdrop">
  <div class="sheet">
    <div class="sheet-head">
      <div style="font-weight:700" id="sheetTitle">Kết nối Wi-Fi</div>
      <button class="x" id="xBtn">✕</button>
    </div>
    <div class="sheet-body">
      <form id="wifiForm">
        <label for="ssid">SSID</label>
        <input id="ssid" name="s" placeholder="Tên Wi-Fi" required>
        <label for="pass">Mật khẩu</label>
        <input id="pass" name="p" type="password" placeholder="Để trống nếu mạng mở">
        <div class="help" id="hint"></div>
      </form>
    </div>
    <div class="footer">
      <button class="btn2" id="cancelBtn">Hủy</button>
      <button class="btn2 primary" id="saveBtn">Kết nối</button>
    </div>
  </div>
</div>

<script>
const list = document.getElementById('list');
const empty = document.getElementById('empty');
const btnRefresh = document.getElementById('btnRefresh');

const sheetWrap = document.getElementById('sheetWrap');
const sheetTitle = document.getElementById('sheetTitle');
const ssidInput = document.getElementById('ssid');
const passInput = document.getElementById('pass');
const hint = document.getElementById('hint');
const xBtn = document.getElementById('xBtn');
const cancelBtn = document.getElementById('cancelBtn');
const saveBtn = document.getElementById('saveBtn');

let currentAP = null;

function wifiSVG(level=1){
  const on = 'stroke:var(--wifi);fill:none';
  const off = 'stroke:var(--wifi-muted);fill:none';
  const dotOn = 'fill:var(--wifi)';
  const dotOff = 'fill:var(--wifi-muted)';

  return `
  <svg viewBox="0 0 24 24" width="28" height="22" aria-hidden="true">
    <path d="M2.5 7.5a14.9 14.9 0 0 1 19 0" style="${level>=4?on:off}" stroke-width="2" stroke-linecap="round"/>
    <path d="M5.5 10.5a10.6 10.6 0 0 1 13 0" style="${level>=3?on:off}" stroke-width="2" stroke-linecap="round"/>
    <path d="M8.6 13.6a6.3 6.3 0 0 1 6.8 0" style="${level>=2?on:off}" stroke-width="2" stroke-linecap="round"/>
    <circle cx="12" cy="17.5" r="1.8" style="${level>=1?dotOn:dotOff}"/>
  </svg>`;
}

function rssiToLevel(rssi){
  if (rssi >= -55) return 4;
  if (rssi >= -65) return 3;
  if (rssi >= -75) return 2;
  return 1;
}

function cell(ap){
  const level = rssiToLevel(ap.rssi);
  const row = document.createElement('button');
  row.className='cell'; row.type='button';
  row.setAttribute('aria-label', 'Chọn mạng '+(ap.ssid||'ẩn'));

  const iconWrap = document.createElement('div');
  iconWrap.className = 'wifi-icon';
  iconWrap.innerHTML = wifiSVG(level);

  if (!ap.open){
    const lock = document.createElementNS('http://www.w3.org/2000/svg','svg');
    lock.setAttribute('viewBox','0 0 24 24');
    lock.setAttribute('class','lock');
    lock.innerHTML = `<path d="M7 11V9a5 5 0 1 1 10 0v2h1a1 1 0 0 1 1 1v7a1 1 0 0 1-1 1H6a1 1 0 0 1-1-1v-7a1 1 0 0 1 1-1h1zm2 0h6V9a3 3 0 0 0-6 0v2z" fill="currentColor"/>`;
    iconWrap.appendChild(lock);
  }

  const center = document.createElement('div'); center.className='grow';
  const t = document.createElement('div'); t.className='ssid'; t.textContent = ap.ssid || '(SSID ẩn)';
  const m = document.createElement('div'); m.className='meta';
  m.textContent = `${ap.open?'Mạng mở':'Có mật khẩu'} • ${ap.rssi} dBm`;
  center.appendChild(t); center.appendChild(m);

  const right = document.createElement('div'); right.className='rssi'; right.textContent = `Mức ${level}/4`;

  row.appendChild(iconWrap); row.appendChild(center); row.appendChild(right);
  row.onclick = () => openSheet(ap);
  return row;
}

async function scan(){
  list.innerHTML='';
  empty.style.display='none';
  btnRefresh.disabled = true; btnRefresh.innerHTML = '<span class="spinner"></span> Đang quét';
  try{
    const r = await fetch('/scan', {cache:'no-store'});
    const data = await r.json();
    const best = {};
    for (const ap of data){
      const k = ap.ssid || '(hidden)';
      if (!best[k] || ap.rssi > best[k].rssi) best[k] = ap;
    }
    const listArr = Object.values(best).sort((a,b)=>b.rssi - a.rssi);
    for (const ap of listArr) list.appendChild(cell(ap));
    empty.style.display = listArr.length? 'none':'block';
  }catch(e){
    empty.style.display='block';
    empty.textContent='Quét thất bại. Hãy thử lại.';
  }finally{
    btnRefresh.disabled=false; btnRefresh.textContent='Quét';
  }
}

function openSheet(ap){
  currentAP = ap || null;
  sheetTitle.textContent = ap ? 'Kết nối: ' + (ap.ssid || '(SSID ẩn)') : 'Nhập SSID thủ công';
  ssidInput.value = ap?.ssid || '';
  ssidInput.readOnly = !!ap?.ssid && ap.ssid.length>0 ? true : false;
  passInput.value = '';
  hint.textContent = ap ? (ap.open ? 'Mạng mở: có thể để trống mật khẩu.' : 'Mạng có mật khẩu: hãy nhập.')
                        : 'Nhập chính xác SSID (phân biệt HOA/thường).';
  sheetWrap.style.display='flex';
  requestAnimationFrame(()=> document.querySelector('.sheet').classList.add('open'));
}
function closeSheet(){
  document.querySelector('.sheet').classList.remove('open');
  setTimeout(()=> sheetWrap.style.display='none', 200);
}
xBtn.onclick = cancelBtn.onclick = ()=> closeSheet();
sheetWrap.addEventListener('click', (e)=>{ if(e.target===sheetWrap) closeSheet(); });

async function save(){
  const s = ssidInput.value.trim();
  const p = passInput.value;
  if(!s){ hint.textContent='Hãy nhập/chọn SSID.'; return; }
  saveBtn.disabled=true; saveBtn.innerHTML='<span class="spinner"></span> Đang kết nối';
  try{
    const fd = new FormData(); fd.append('s', s); fd.append('p', p);
    const r = await fetch('/save', {method:'POST', body:fd});
    const t = await r.text();
    hint.textContent = t || 'Đã gửi cấu hình. Thiết bị sẽ khởi động lại…';
  }catch{
    hint.textContent = 'Không gửi được dữ liệu.';
  }finally{
    saveBtn.disabled=false; saveBtn.textContent='Kết nối';
    setTimeout(()=> location.reload(), 2500);
  }
}
saveBtn.onclick = save;
btnRefresh.onclick = scan;
scan();
</script>
</body>
</html>
  )rawliteral";
}

// ========== Hàm nội bộ ==========
static bool hasStoredCredentials(String &ssid, String &pass) {
  prefs.begin("wifi", true);
  ssid = prefs.getString("ssid", "");
  pass = prefs.getString("pass", "");
  prefs.end();
  return ssid.length() > 0;
}

void connectWiFi(const String &ssid, const String &pass, uint32_t timeout_ms) {
  WiFi.mode(WIFI_STA);
  WiFi.setAutoReconnect(true);
  if (pass.length()) WiFi.begin(ssid.c_str(), pass.c_str());
  else WiFi.begin(ssid.c_str());

  uint32_t start = millis();
  while (WiFi.status() != WL_CONNECTED && (millis() - start) < timeout_ms) {
    delay(250);
  }
}

void handleSensors()
{
  float t = global_temperature;
  float h = global_humidity;
  String json = "{\"temp\":" + String(t) + ",\"hum\":" + String(h) + "}";
  server->send(200, "application/json", json);
}
static void handleRoot() {
  server->send(200, "text/html", mainPage()); 
}
void handleSettings() { 
  String html = settingsPage();
  String ipStr = (WiFi.getMode() & WIFI_STA) && (WiFi.status() == WL_CONNECTED)
                 ? WiFi.localIP().toString()
                 : WiFi.softAPIP().toString();
  html.replace("%IP%", ipStr);
  server->send(200, "text/html", html); 
}

static void handleScan() {
  int n = WiFi.scanNetworks(false, true);
  String json = "[";
  for (int i = 0; i < n; i++) {
    if (i) json += ",";
    String ssid = WiFi.SSID(i);
    int32_t rssi = WiFi.RSSI(i);
    wifi_auth_mode_t enc = WiFi.encryptionType(i);
    bool open = (enc == WIFI_AUTH_OPEN);
    ssid.replace("\"", "\\\"");
    json += "{\"ssid\":\"" + ssid + "\",\"rssi\":" + String(rssi) + ",\"open\":" + (open ? "true" : "false") + "}";
  }
  json += "]";
  server->send(200, "application/json; charset=utf-8", json);
  WiFi.scanDelete();
}

static void handleSave() {
  if (!server->hasArg("s")) {
    server->send(400, "text/plain; charset=utf-8", "Thiếu SSID.");
    return;
  }
  String ssid = server->arg("s");
  String pass = server->hasArg("p") ? server->arg("p") : "";
  prefs.begin("wifi", false);
  prefs.putString("ssid", ssid);
  prefs.putString("pass", pass);
  prefs.end();
  server->send(200, "text/html; charset=utf-8",
              "Đã lưu cấu hình.");
  connectWiFi(ssid,pass,20000);
  isAPMode = false;
  connecting = true;
  delay(2000);
}

static void startConfigPortal() {
  WiFi.mode(WIFI_AP);
  WiFi.softAP(current_ap_ssid.c_str(), current_ap_pass.c_str());
  delay(100);
  IPAddress ip = WiFi.softAPIP();
  isAPMode = true;
  connecting = false;
  if (!server) {
    server = new WebServer(80);
  }

  server->on("/", HTTP_GET, handleRoot);
  server->on("/settings", HTTP_GET, handleSettings);
  server->on("/scan", HTTP_GET, handleScan);
  server->on("/save", HTTP_POST, handleSave);
  server->onNotFound([](){ server->send(404, "text/plain; charset=utf-8", "Not found"); });
  server->begin();

  Serial.println("=== CONFIG PORTAL ===");
  Serial.printf("AP SSID: %s\n", current_ap_ssid.c_str());
  Serial.printf("AP IP  : %s\n", ip.toString().c_str());
  Serial.println("Mở: http://192.168.4.1");

  // Vòng lặp phục vụ portal (blocking)
}

static void printNetworkInfo() {
  Serial.println("=== WIFI CONNECTED ===");
  Serial.printf("SSID: %s\n", WiFi.SSID().c_str());
  Serial.print("IP: "); Serial.println(WiFi.localIP());
  Serial.print("GW: "); Serial.println(WiFi.gatewayIP());
  Serial.print("RSSI: "); Serial.println(WiFi.RSSI());
}

// ========== API công khai ==========

void wifi_clear_credentials() {
  prefs.begin("wifi", false);
  prefs.clear();
  prefs.end();
  Serial.println("[WiFiManager] Credentials cleared");
}

bool wifi_is_connected() {
  return WiFi.status() == WL_CONNECTED;
}

bool wifi_get_credentials(String &ssid, String &pass) {
  return hasStoredCredentials(ssid, pass);
}

void main_server_task(void *pvParameters)
{
  startConfigPortal();
  while (1)
  {
    server->handleClient();
    // BOOT Button to switch to AP Mode
    if (digitalRead(BOOT_PIN) == LOW)
    {
      vTaskDelay(100);
      if (digitalRead(BOOT_PIN) == LOW)
      {
        if (!isAPMode)
        {
          startConfigPortal();
        }
      }
    }

    // STA Mode
    if (connecting)
    {
      if (WiFi.status() == WL_CONNECTED)
      {
        Serial.print("STA IP address: ");
        Serial.println(WiFi.localIP());
        isWifiConnected = true; // Internet access
        connecting = false;
        xSemaphoreGive(xBinarySemaphoreInternet);
        isAPMode = false;
      }
      else if (millis() - connect_start_ms > 10000)
      { // timeout 10s
        Serial.println("WiFi connect failed! Back to AP.");
        startConfigPortal();
        connecting = false;
        isAPMode = true;
        isWifiConnected = false;
      }
    }

    vTaskDelay(20); // avoid watchdog reset
  }
}